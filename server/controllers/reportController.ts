...
import type { ReportType, ReportStore } from "../reports/reportStore.js";
import { serializeReport } from "../serializers/reportResponse.js";

export function createReportHandlers(reports: ReportStore) {
  ...
  const claimReport = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    try {
  ...
  const resolveReport = async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    try {
  ...
  const getGlobalStats = async (_req: Request, res: Response) => {
    try {
  ...
  return { createReport, getAllReports, getMyReports, claimReport, resolveReport, getGlobalStats, getUserStats };
}

export const { createReport, getAllReports, getMyReports, claimReport, resolveReport, getGlobalStats, getUserStats } = createReportHandlers(mongoReportStore);
